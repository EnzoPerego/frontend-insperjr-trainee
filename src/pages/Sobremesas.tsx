...
export default function Sobremesas(): React.JSX.Element {
  const [activeSection, setActiveSection] = useState<string>('sobremesas')
  ...
  useEffect(() => {
    const loadProdutos = async (): Promise<void> => {
      ...
      try {
        ...
        const cat = categorias.find(c => c.nome.toLowerCase() === 'sobremesas')
        if (!cat) {
          setProdutos([])
        } else {
    ...
    >
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold text-gray-900">Sobremesas</h1>
            <p className="text-gray-600 mt-1">Gerencie as sobremesas do cardápio</p>
          </div>
          <a href="/admin/cardapio/sobremesas/novo" className="px-4 py-2 bg-kaiserhaus-dark-brown text-white rounded-lg hover:bg-kaiserhaus-brown transition-colors font-medium">
            Adicionar Sobremesa +
          </a>
        </div>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="mt-2 flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <a href={`/admin/cardapio/sobremesas/editar?id=${produto.id}`} className="px-3 py-1.5 rounded-md text-sm text-blue-700 bg-blue-50 hover:bg-blue-100 transition-colors">
                            Editar
                          </a>
                        </div>
                      </div>
                    ...
                    {produtos.map((produto) => (
                      <tr key={produto.id} className={produto.status === 'Inativo' ? 'opacity-60' : ''}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium flex items-center gap-2">
                          ...
                          <a href={`/admin/cardapio/sobremesas/editar?id=${produto.id}`} className="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Editar">
                            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
